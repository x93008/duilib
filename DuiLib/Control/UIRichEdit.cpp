...
private:
	...
	unsigned    fNeedFreshCaret     :1; // 控制文本框光标重新定位原因，可能不需要刷新操作
...
{
    //Fixed Issue In Win 7，当调用Invalidate修改rcClient，可能界面可能会被裁剪
    RECT rc = {};
    if (prc == NULL)
        rc = rcClient;
    else
...
// 这是非rich格式的richedit的一个滚动条bug，当滚动一行，且可见时，LineDown或SetScrollPos无法正确滚动内容
// 所以iPos参数为滚动条解决bug
