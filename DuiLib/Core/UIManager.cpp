    ...
    case WM_MOUSEHOVER:
        {
            ...
            // by jiangdong 2016-8-6 修改tooltip 悬停时间 闪烁bug
            ...
            //修改在CListElementUI 鼠标显示 光标问题显示手无法正确移动，但是实际上应该移动的
    ...
    case WM_LBUTTONUP:
        {
			...
			// 在Click事件中的弹出模态对话框退出阶段触发实例，可能已经删除
			// this的成员变量赋值可能导致heap错误
			// this的成员变量设置将会导致野指针异常
			// 使用栈上的成员变量进行响应应该当前粘贴成员
			// 在弹出对话模态窗口返回时，此栈阶段不存在任何类实例，可能会造成
			// 错误会触发异常
...
bool CPaintManagerUI::TranslateMessage(const LPMSG pMsg)
{
	...
	if (uChildRes != 0)
	{
		...
		//code by redrain 2014.12.3,修复edit和webbrowser的tab无法切换焦点的bug
		//		for( int i = 0; i < m_aPreMessages.GetSize(); i++ ) 
