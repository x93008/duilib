...
void CPage1::OnClick(TNotifyUI& msg)
{
	...
	{                
		...
		{
			MessageBox(NULL, 
				find_ctrl->GetText()+_T(" 演示未选择行时的按钮，鼠标进入点击按钮会找到所在行的listcontainer.."), 
				_T("DUILIB DEMO"), MB_OK);   
			((CLabelUI *)find_ctrl)->SetText(_T("可成功动态增加控件..."));
		}
		}
...
void CRichListWnd::OnClick( TNotifyUI &msg )
{
	...
	{ 
		PostQuitMessage(0); // 因为activex的原因使用close可能会重复执行
		return; 
...
void CRichListWnd::OnItemClick( TNotifyUI &msg )
{
	...
	wsprintf(alert_msg, _T("选择行号%d, 右击可操作该行的所有子项..."), index);
	...
	{
		MessageBox(NULL, 
			find_ctrl->GetText()+_T(" 选择行的所有子项操作.."), 
			_T("DUILIB DEMO"), MB_OK);   
		((CLabelUI *)find_ctrl)->SetText(_T("可成功动态增加控件..."));
	}
	else
	{
		MessageBox(NULL, _T("或者容器未为控件设置name，无法找到需要操作的控件"), 
			_T("DUILIB DEMO"), MB_OK);   
	}
	...
	if(find_ctrl)
	{
		...
		wsprintf(alert_msg, _T("当前值:%d"), ((CProgressUI *)find_ctrl)->GetValue());
		MessageBox(NULL, alert_msg, _T("DUILIB DEMO"), MB_OK);   

		((CProgressUI *)find_ctrl)->SetValue(30);
		MessageBox(NULL, _T("修改了进度值"), _T("DUILIB DEMO"), MB_OK);   
	}
}
...
LRESULT CRichListWnd::OnMouseHover(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	/*演示暂停下载列表项图标，此时动态改变其图标状态演示*/
	if(pHover->GetName() == _T("down_ico"))
	{
		MessageBox(NULL, _T("演示某个控件如按钮暂停后，动态控件属性会改变其状态图标等"), _T("DUILIB DEMO"), MB_OK);
...
LRESULT CRichListWnd::OnChar( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	{
		MessageBox(NULL, _T("演示了回调机制"), _T("DUILIB DEMO"), MB_OK);
	}
	else
	{
		bHandled = FALSE;
