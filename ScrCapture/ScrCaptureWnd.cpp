...
void CScrCaptureWnd::Init() 
{
	...
	::AppendMenu(m_hEditMenu, MF_STRING, MENUCMD_ADDBRUSH, _T("添加画笔描述"));
	...
	::AppendMenu(m_hEditMenu, MF_STRING, MENUCMD_RECHOICE, _T("重新选择图像区域"));
	::AppendMenu(m_hEditMenu, MF_STRING, MENUCMD_SAVEANDQUIT, _T("保存并退出截图"));
	::AppendMenu(m_hEditMenu, MF_STRING, MENUCMD_SAVE, _T("保存"));
	::AppendMenu(m_hEditMenu, MF_SEPARATOR, 0, NULL);
	::AppendMenu(m_hEditMenu, MF_STRING, MENUCMD_HIDETOOLBAR, _T("隐藏编辑工具栏"));
	::AppendMenu(m_hEditMenu, MF_STRING, MENUCMD_QUIT, _T("退出截图"));
...
LRESULT CScrCaptureWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if( uMsg == WM_CREATE ) {
		...
		if (!m_bClipChoiced) {
			if( ::GetKeyState(VK_LBUTTON) < 0 ) { // 鼠标按下状态
